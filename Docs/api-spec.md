...
- `title` は必須です。
- `isbn` は未入力、または有効なISBN-10/ISBN-13のみ許可し、保存時は数字と `X` のみに正規化します。
- `publishedDate` は未入力、`YYYY`、`YYYY-MM`、`YYYY-MM-DD` のいずれかを許可します。`YYYY.M` は `YYYY-MM` に正規化します。
- `locationId` は有効な保管場所タグIDのみ許可します。
- `classificationTagIds` は有効な分類タグIDのみ許可します。
...
- `isbn` は未入力、または有効なISBN-10/ISBN-13のみ許可し、保存時は数字と `X` のみに正規化します。
- `publishedDate` は未入力、`YYYY`、`YYYY-MM`、`YYYY-MM-DD` のいずれかを許可します。`YYYY.M` は `YYYY-MM` に正規化します。
- `locationId` は有効な保管場所タグIDのみ許可します。ただし、既にその本に紐づいている無効化済み保管場所タグを維持する更新は許可します。
- `classificationTagIds` は有効な分類タグIDのみ許可します。ただし、既にその本に紐づいている無効化済み分類タグを維持する更新は許可します。
...
- NDLサーチのsubjectは、`xsi:type` や `rdf:datatype` などの型指定がない主題語のみを分類タグ候補として扱います。
- NDLサーチの`dcndl:genre`は、ジャンル・形式用語として分類タグ候補に含めます。
- NDLサーチの`dcndl:NDC9`またはNDC9分類URIのsubjectは、日本図書館協会のNDC9 Linked Dataを照合し、文脈付き分類項目名を分類タグ候補に含めます。
- NDC8、NDC10、版不明のNDC、NDLCなどの未対応分類記号と、NDC9マスタに存在しない分類記号は候補に含めません。未解決のコードをそのままタグ名として返すこともありません。
- Open Library APIのsubjectsは分類タグ候補として扱い、ユーザーが確定したものだけ保存します。
...
- Import payload内で、ID、管理用バーコード、保管場所タグ名、分類タグ名、または本と分類タグの同一関連ペアが重複する場合は、プレビュー時点でエラーとして返します。
...
- Import payload内のID、管理用バーコード、保管場所タグ名、分類タグ名、または本と分類タグの同一関連ペアの重複も、ユーザー選択では解決せず `400 Bad Request` として返します。
- 既存データとの重複はエラーではなく、Importプレビューで競合として返します。
