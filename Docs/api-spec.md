...
- ISBNとして解釈できない値の場合は外部APIを呼び出さず、手入力登録に進めます。
...
- 外部APIの利用は低頻度のリアルタイム照会に限定し、同じISBNやバーコードの照会結果はキャッシュします。
- 現状のキャッシュはBackendプロセス内のインメモリキャッシュです。
- SQLiteへの永続キャッシュは後続Issueで実装します。
